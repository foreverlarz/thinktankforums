...
$sql = "SELECT name FROM ttf_forum WHERE forum_id='".clean($forum_id)."'";
...
$sql = "INSERT INTO ttf_thread SET forum_id='".clean($forum_id)."', ".
       "author_id='{$ttf["uid"]}', date=UNIX_TIMESTAMP(), title='".clean($title)."'";
if (!$result = mysql_query($sql)) showerror();
$thread_id = mysql_insert_id();
                
// update the date, thread count, and post count of the forum
$sql = "UPDATE ttf_forum SET threads=threads+1 WHERE forum_id='".clean($forum_id)."'";
if (!$result = mysql_query($sql)) showerror();
